...
[*Photo: Full device assembly with all components labeled*]
...
[*Diagram: Relay module wiring diagram with actuator labels*]
...
### What to Expect on the Touchscreen

[*Screenshot: MASH web dashboard shown on the Raspberry Pi 7" touchscreen*]

On first boot with no configured Wi-Fi, the dashboard will display an **orange provisioning banner** indicating the device is in hotspot mode. Follow the steps in [Chapter 4](#4-wifi-provisioning-first-time-setup) to connect the device to your local network.
...
[*Screenshot: Phone Wi-Fi settings showing "MASH-Device" network*]
...
[*Screenshot: Browser showing the WiFi Setup page at 10.42.0.1:5000/wifi-setup*]
...
[*Screenshot: WiFi Setup page with network dropdown open*]
...
[*Screenshot: "Connect to Network" button being tapped, then the "Applying Settings…" page*]

Tap **Connect to Network**. The button will disable itself after the first tap to prevent duplicate submissions.

The browser will redirect to the **"Applying Settings…** page, which shows:
- An animated spinner
- The target network SSID in a badge
- Instructions explaining what happens next
...
[*Screenshot: Browser address bar with device IP entered*]
...
[*Screenshot: Left sidebar navigation showing all 5 links*]
...
[*Screenshot: Dashboard showing Fruiting Room with Optimal condition badge*]
...
[*Screenshot: Orange provisioning banner with "WiFi Setup" button*]
...
[*Screenshot: Three sensor cards — CO₂, Temperature, Humidity — with live values*]
...
Each card shows:
- The **live reading** in large text.
- The **target value** configured for that room in the card footer.
- A **Room Condition badge** in the room header, indicating the overall state:
...
[*Screenshot: Actuator icon strip below sensor cards, some icons lit green*]
...
[*Screenshot: QR code modal opened from the floating button, showing connection QR*]
...
[*Screenshot: Controls page showing Fruiting Room actuator cards in Manual mode*]
...
[*Screenshot: Fruiting Room actuator cards — all five displayed in a grid*]
...
> **Cooldown:** After toggling any actuator, a **5-second cooldown** applies to that card. This prevents rapid toggling that could damage relay hardware. The card will be unresponsive during the cooldown and a countdown indicator is shown.
...
[*Screenshot: Alerts page with both Active Alerts and Alert History sections*]
...
[*Screenshot: Active Alerts table with one Critical row highlighted in red*]
...
| **Message**     | Human-readable description of the condition (e.g., "CO₂ level exceeds critical threshold") |
| **Detected At** | Timestamp when the alert was first triggered                                               |
| **Action**      | Acknowledge button                                                                         |

**To acknowledge an alert:**
1. Once the condition that caused the alert has been resolved (e.g., you have manually ventilated the room), tap the **Acknowledge** button on that row.
...
[*Screenshot: AI Insights page showing both model status cards as "Active"*]
...
Two model cards display the status of each ML component:
...
[*Screenshot: Settings page with the General tab active*]

### Tab 1 — General

[*Screenshot: General tab showing current time and system uptime*]
...
[*Screenshot: WiFi Connection tab showing connected SSID and Disconnect button*]
...
[*Screenshot: Cloud Sync tab with Firebase toggle enabled*]
...
When Firebase Sync is **enabled**, all sensor readings and actuator state changes are pushed to the MASH Firebase Realtime Database in real time. This enables:
- Remote monitoring via the MASH Grower Mobile app (Cloud Mode).
- Historical data access from anywhere with internet access.
- Real-time alert delivery to connected mobile apps.

When Firebase Sync is **disabled**, all data continues to be saved locally to the device's SQLite database, but no cloud data is transmitted. The MASH Grower Mobile app can still connect in Local Mode (same Wi-Fi network).

### Tab 4 — Power Controls

[*Screenshot: Power Controls tab showing Reboot and Shutdown buttons*]

> **Warning:** Both actions on this tab will temporarily interrupt all environment monitoring. Ensure cultivation conditions are stable — or that you can manually oversee the chambers — before proceeding.
...
[*Screenshot: About System tab showing version info, Update Status badge, and Check for Updates button*]
...
| **History**           | Opens the Changelog modal showing all 20+ release notes       |
...
[*Screenshot: Mobile app "Connect Your Chamber" screen with MASH ID field*]
...
[*Screenshot: Dashboard floating QR button tapped, showing QR code modal on the device screen*]
...
The QR code contains the device's MASH ID, name, local IP address, and port. The app uses this information to immediately initiate connection — no manual entry required.
...
The active connection mode is shown in the mobile app's header after pairing.
...
Each incoming sensor reading is first evaluated for statistical validity. If a reading is flagged as anomalous (e.g., a CO₂ spike to 9,999 ppm caused by a sensor glitch), the system substitutes the last known valid reading to prevent false actuation. This ensures the automation engine always acts on reliable data.

**Stage 2 — Actuation Decision (Decision Tree)**
The validated readings — temperature, humidity, CO₂ concentration, and current time of day — are passed to the Decision Tree model. The model outputs a recommended state (ON or OFF) for each actuator based on patterns learned from historical sensor data collected in this facility.
...
| CO₂ recovered           | CO₂ drops below 900 ppm (1,000 − 100 hysteresis) | Turn Exhaust Fan OFF                                      |
...
When humidity falls below 85 %, the system starts an alternating mist/fan cycle rather than running both simultaneously (which would saturate the sensor). The cycle continues until humidity reaches the 90 % target.
...
The cycle repeats automatically. A trend-based predictor monitors the rate of humidity change across the last three readings; if humidity is rising rapidly toward 95 %, the cycle stops early to prevent overshoot.
...
3. Overrides **expire automatically after 5 minutes**, after which the automation engine resumes control of that actuator.
4. All overrides are cleared immediately when the mode is switched back to Automatic.

### Automation Startup Delay

After powering on, MASHAuto will not begin issuing commands until the **30-second sensor warmup period** has completed. This prevents actuator activity during initial sensor stabilization.
...
[*Screenshot: Settings → About System tab showing "Update Available" badge*]

1. Navigate to **Settings → About System** tab.
2. Tap **Check for Updates**. The device will query the GitHub Releases API.
3. The **Update Status** badge will change:
   - **"Up to Date"** (green) — The installed firmware is the latest version.
   - **"Update Available"** (orange or red) — A newer version is available.

Updates are also checked automatically in the background; the badge in the About System tab will reflect the latest status without needing a manual check.
...
Before installing an update, tap the **History** button on the About System tab to open the Changelog modal. This shows the release notes for all versions, including the pending update. Review the "What's New" section to understand what the update contains.

[*Screenshot: Changelog modal scrolled to show the latest version entry*]

### Installing an Update

[*Screenshot: Update Available modal with "Update Now" button highlighted*]

1. When an update is available, a modal appears with:
   - The new version number and release name.
   - The changelog for that version.
   - A warning: **"The device will restart automatically after the update."**

2. Tap **Update Now** to begin. A progress bar will display the download progress.

3. Once downloaded, the new firmware is applied and the device **restarts automatically**. All monitoring resumes within ~60 seconds after the restart.

> **Rollback Safety:** The update system uses an A/B rollback mechanism. If the new firmware fails to boot successfully, the device automatically rolls back to the previous stable version and marks the failed version as unstable to prevent it from being installed again.

> **Tap "Later"** to dismiss the update modal and install at a more convenient time.
...
> *Symptom:* The `MASH-Device` hotspot keeps reappearing after attempting WiFi setup.

- Verify that the network name (SSID) was entered correctly, including capitalization.
- Confirm the password is correct by connecting another device to the same network with the same credentials.
...
- If the hotspot does not automatically restart after a failed attempt, reboot the device using the button on the touchscreen (Settings → Power Controls → Reboot).

---

**2. The web interface is not loading in my browser.**

> *Symptom:* Browser shows "This site can't be reached" when navigating to the device IP.
...
- Confirm you are accessing port **5000**: `http://<ip>:5000` — not port 80 or 443.
- Try obtaining the device IP from your router's admin panel (look for a device named "raspberrypi" or the device's MAC address).
- If no IP is found, the device may be in hotspot mode — connect to `MASH-Device` (no password) and access `http://10.42.0.1:5000/wifi-setup` to reconfigure.

---

**3. Sensor readings on the dashboard show 0 or "—".**

> *Symptom:* Sensor cards show zero values or dashes; Arduino status shows "Offline."

- Check that the Arduino Uno is powered (its power LED should be lit) and connected to the Raspberry Pi via USB.
- Verify the USB cable is seated firmly in both the Arduino and the RPi.
- The Arduino status indicator in the top status bar shows "Connected" when the serial link is active. If it shows "Offline," the serial connection has been lost.
...
> *Symptom:* Tapping an actuator card in Manual mode does not trigger the relay.

- Confirm the Auto/Manual toggle at the top of the Controls page is set to **Manual Control** (toggle OFF / grey). In Automatic mode, cards cannot be clicked.
- Wait for the **5-second cooldown** to expire if the actuator was toggled recently.
- Verify the Arduino is connected (sensor readings should be updating on the Dashboard). If sensors are offline, the serial link is down and commands cannot be sent.
- Check that the relay module is powered. Actuators will only respond if the relay board is receiving power.

---

**5. MASHAuto automation does not appear to be running.**

> *Symptom:* Controls page shows Manual mode, or no automation actions are occurring despite out-of-range sensor readings.

- Open the Controls page and confirm the toggle at the top reads **Automatic Control** (and is green/ON).
- Wait 30–60 seconds after switching to Auto mode for the warmup period to complete.
- Verify that ML is enabled in the device configuration (`ml_enabled: true` in `config/config.yaml`). Check the AI Insights page to confirm both model status cards show "Model Active."
- Confirm sensor readings are live (non-zero, updating in Dashboard). Automation cannot run without valid sensor input.

---

**6. The QR code on the dashboard is not scannable from the mobile app.**

> *Symptom:* Mobile app camera opens but does not detect the QR code on the touchscreen.
...
- Reduce ambient glare on the screen (adjust room lighting or screen brightness).
- As an alternative, use the **MASH ID manual entry** method to pair the mobile app. The MASH ID is shown in Settings → About System.

---

**7. Cannot connect the mobile app to the device.**

> *Symptom:* Mobile app shows "Device Not Reachable" or connection attempts time out.

- For **Local Mode**: ensure the mobile device and the MASH Device are on the same Wi-Fi network. The app communicates with the device at its local IP address on port 5000.
- For **Cloud Mode**: ensure both the MASH Device and the mobile device have active internet connections.
- Verify the device's Firebase cloud sync is enabled (Settings → Cloud Sync → toggle ON).
- Confirm the MASH ID entered in the mobile app exactly matches the serial number shown in Settings → About System.

---

**8. Firebase cloud sync is not working.**

> *Symptom:* "Sync" indicator on the Dashboard shows "Inactive"; mobile app cannot see live data in Cloud Mode.
...
- The device's Firebase credentials are stored in `config/firebase_config.json`. If this file is missing or corrupted, sync will fail. Contact the system administrator to restore the configuration.

---

**9. The device is not rebooting after selecting "Reboot Device."**

> *Symptom:* The web interface seems to freeze or the device does not come back online.
...
- If the device does not come back online after 2 minutes, perform a manual power cycle by disconnecting and reconnecting the power supply.

---

**10. A firmware OTA update failed.**

> *Symptom:* Update process started but the device is unresponsive, or the version number has not changed after restart.

- The device's A/B rollback mechanism should automatically restore the previous firmware version if the new version fails to boot. After the rollback completes, the device will restart with the prior firmware.
...
- If rollback also fails, contact the development team for manual firmware recovery instructions.
...
| Parameter         | Target      | Min     | Max       | Alert Tolerance | CO₂ Trigger                   |
| ----------------- | ----------- | ------- | --------- | --------------- | ----------------------------- |
...
| Humidifier Fan          | Fruiting         | 5           | 4       | Distributes mist from the Mist Maker throughout the chamber     |
| Mist Maker (Humidifier) | Fruiting         | 6           | 5       | Generates ultrasonic mist to raise relative humidity            |
| LED Grow Lights         | Fruiting         | 7           | 6       | Provides grow lighting; follows timed 08:00–20:00 schedule      |
| Spawning Exhaust Fan    | Spawning         | 2           | 1       | Provides passive ventilation and CO₂ flush for spawning chamber |
